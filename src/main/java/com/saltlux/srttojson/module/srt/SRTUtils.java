...
public class SRTUtils {
    ...
    public static long textTimeToMillis (final String time) throws IllegalArgumentException {

        Matcher matcher = PATTERN_TIME.matcher(time);
        if (isTimeformatCorrect(time, matcher)) {
            throw new IllegalArgumentException("incorret time format...");
        }
...
//
     ...
     * @return
     */
    public static boolean speedSynchronization (final ArrayList<Subtitle> listSubtitles, long timeInMillis, File fileOut) throws IOException {
...
//			fileOut.mkdirs();
    ...
    private static boolean isTimeformatCorrect(String time, Matcher matcher) {
        return time == null || time.isEmpty() || !matcher.find();
    }
}
